...
const sourceClientPath = resolve(productRoot, "dist/client.js");
if (!existsSync(sourceClientPath)) {
  throw new Error("Missing packages/node/dist/client.js; run `bun run build:judge` before proving judge sensitivity");
}
const temporaryRoot = await mkdtemp(join(tmpdir(), "ytm-judge-broken-"));

try {
  await cp(productRoot, temporaryRoot, { recursive: true });
  const clientPath = resolve(temporaryRoot, "dist/client.js");
  ...
  await writeFile(clientPath, source.replace(marker, "return { ...envelope.value, source: null };"));
  ...
  if (!proof.stderr.includes("matrix-success: node public result differs from the approved golden result")) {
